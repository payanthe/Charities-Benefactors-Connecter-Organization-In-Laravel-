...
<div class="donatorsHolder col-12 pb-2 " style="background: rgba(255,255,255,0.7);padding: 50px 0 25px 0;">
    ...
    <div class="container col-12 row justify-content-end">
        <form class="col-4 row justify-content-end mb-2" method="post" action="{{url('/benfilter')}}">
            {{csrf_field()}}
            <select class="form-control col-6" name="rqdAb">
                <option value="0">Any </option>
            <?php $number = 1; ?>
                @foreach(Auth::user()->charAbilities as $ability)
                    <option value="{{$ability->id}}">{{$number++.'.'}} {{$ability->name}} </option>
                @endforeach
            </select>
            <button class="form-control col-2" type="submit"><i class="fas fa-filter"></i></button>
        </form>
    </div>
    <div class="donatorsBar row justify-content-around">
        ...
        <div class="charitiesBox col-2 justify-content-start p-1 ml-1 benefactorBoxes"
             style="background:rgba(249,249,249,0.89);border-radius: 5px;height:420px;">
            <img src="img/Dn/images.jpg" style="width: 100%">
            <div class="content row justify-content-start" style="height: 155px;
                border-bottom: 2px dotted #2e5a73;">
                <span class="text-left pl-1">{{$benefactor->name}}</span><br>
                ...
                <div class="col-12 text-left" style="height: 150px">
            ...
            <a class="btn button col-12" style="color: white" href="{{url('benefactors/profile/'.$benefactor->id)}}">Visit</a>
        </div>
        @endforeach
    </div>
</div>
